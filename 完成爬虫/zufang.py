...
def drop_data():
    print("执行删除重复数据...")
    con = conn.cursor()
    drop_sql = "DELETE from zufang_copy WHERE (qu, name,huxing,mianji,chaoxiang) in " \
               "(SELECT qu, name,huxing,mianji,chaoxiang from " \
               "(SELECT qu, name,huxing,mianji,chaoxiang FROM zufang_copy " \
               "GROUP BY qu, name,huxing,mianji,chaoxiang HAVING COUNT(*)>1) s1) AND  id NOT in " \
               "(SELECT id from (SELECT id FROM zufang_copy " \
               "GROUP BY qu, name,huxing,mianji,chaoxiang HAVING COUNT(*)>1) s2)"
...
thread11 = threading.Thread(target=parse, args=('https://bj.lianjia.com/zufang/fangshan/pg', '房山区'))
